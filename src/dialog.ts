...
type ButtonsRecord = Record<
  string,
  | ClickEventHandler
  | { action?: ClickEventHandler; form?: string; submit?: boolean }
>;
...
const { defaultTitles, defaultContent } = pageProps.dialog as unknown as Record<
  string,
...
class DialogManager {
  ...
  private display<T extends DialogParams["type"]>({
    ...
    if (appendingToRequest) {
      $requestContentDiv = this.$dialogContent
        .children(":not(#dialogButtons)")
        .last();
      let $errorNotice = $requestContentDiv.children(".notice:last-child");
      if (!$errorNotice.length) {
        $errorNotice = $(document.createElement("div")).append(
          $(document.createElement("p")).attr("class", "mb-0"),
        );
    ...
    if (!appendingToRequest) {
      this.$dialogHeader
        ...
        .attr(
          "class",
          `modal-header${params.color ? ` bg-${params.color} text-${yiq[params.color]}` : ""}`,
        );
    }
      ...
      $.each(params.buttons, (name, obj) => {
        ...
        if (typeof obj !== "function" && "form" in obj) {
          ...
          if ($contentDiv.length === 1) {
            ...
            $contentDiv.prepend(
              $(document.createElement("input")).attr("type", "submit").hide(),
            );
          }
        }
        $button
          ...
          .on("keydown", (e) => {
            if ([Key.Enter, Key.Space].includes(e.keyCode)) {
              ...
              [Key.Tab, Key.LeftArrow, Key.RightArrow].includes(e.keyCode)
            ) {
              e.preventDefault();
              ...
              if ($focused.length) {
                if (!e.shiftKey) {
                  ...
                  else if (isKey(Key.Tab, e))
                    $inputs.add($dBc).filter(":visible").first().focus();
                } else if ($focused.prev().length) $focused.prev().focus();
                else if (isKey(Key.Tab, e))
                  ($inputs.length > 0 ? $inputs : $dBc)
                    .filter(":visible")
                    .last()
                    .focus();
              } else
                $inputs.add($dBc)[!e.shiftKey ? "first" : "last"]().focus();
            }
          })
          .on("click", (e) => {
            e.preventDefault();
    ...
    if (!append) {
      ...
      this.modalInstance = new Modal(overlayEl, {
        backdrop: "static",
        keyboard: false,
      });
      ...
      const onModalShown = () => {
        ...
        if (append) {
          ...
          this.$dialogOverlay.stop().animate(
            {
              scrollTop: `+=${
                $lastdiv.position().top +
                parseFloat($lastdiv.css("margin-top")) +
                parseFloat($lastdiv.css("border-top-width"))
              }`,
            },
            "fast",
          );
  ...
  wait(title: string | false, additionalInfo?: string) {
    const localAdditionalInfo =
      typeof additionalInfo !== "string" ? defaultContent.wait : additionalInfo;
    this.display({
      type: "wait",
      title,
  ...
  request({
    ...
    if (confirmBtn !== false) {
      if (formId)
        buttons[confirmBtn || pageProps.dialog.submit] = {
          submit: true,
          form: formId,
        };
      buttons[pageProps.dialog.cancel] = () => {
        this.close();
      };
    } else
      buttons[pageProps.dialog.close] = {
        action: () => this.close(),
        form: formId,
      };
  ...
  confirm({
    ...
    const localBtnTextArray = Array.isArray(btnTextArray)
      ? btnTextArray
      : [pageProps.dialog.yes, pageProps.dialog.no];
  ...
  private setFocus() {
    ...
    else if ($actions.length > 0 && this.open?.type !== "info")
      $actions.first().focus();
  }
  ...
  clearNotice(regexp?: RegExp): boolean {
    const $notice = this.$dialogContent
      .children(":not(#dialogButtons)")
      .children(".notice:last-child");
