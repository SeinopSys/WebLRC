...
const isClipboardEvent = (e: unknown): e is ClipboardEvent =>
  typeof e === "object" &&
  e !== null &&
  "clipboardData" in e &&
...
$(document.body)
  .on("paste", "[contenteditable]", function (e) {
    ...
    if (isClipboardEvent(e) && e.clipboardData)
      ...
      isClipboardEvent(e.originalEvent) &&
      e.originalEvent.clipboardData
    ) {
      text = $(document.createElement("div")).text(
    ...
    if (document.queryCommandSupported("insertText")) {
      document.execCommand(
        "insertHTML",
        false,
        typeof text === "string" ? text : $(text).html(),
      );
      e.preventDefault();
      e.stopPropagation();
      return;
