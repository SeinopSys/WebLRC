...
import { Reader as JsMediaTagsReaderReal } from "jsmediatags/dist/jsmediatags.min";
import type { Reader as JsMediaTagsReader } from "jsmediatags";
import type { Tags as JsMediaTagsTags } from "jsmediatags/types";
import { Dialog } from "../dialog";
import { pad, rangeLimit, roundTo, setElDisabled } from "../utils";
...
export class AudioPlayer {
  ...
  constructor(private pluginScope: { editor: TimingEditor }) {
    ...
    $(document).on("mouseup", (e) => {
      if (
        e.button !== 0 ||
        !this.stateInd.$progressWrap.hasClass("moving") ||
        !this.hasFile()
      )
        return;
    ...
    this.$volumedown.on("click", (e, shiftKey, altKey) => {
      ...
      const newvol =
        ...
        (e.altKey || altKey
          ? 0.01
          : this.volumeStep * (e.shiftKey || shiftKey ? 2 : 1));
  ...
  seek(seconds: number): void {
    ...
    this.audio.currentTime = rangeLimit(
  ...
  async setFile(file?: File | null): Promise<File> {
    ...
    if (!file || !/^(?:audio\/|video\/ogg)/.test(file.type)) {
      Dialog.fail(
        pageProps.jsLocales.dialog_format_error,
        pageProps.jsLocales.dialog_format_notaudio,
      );
      return Promise.reject(
        new Error(`[setFile] Audio file expected, got ${file?.type}`),
      );
    }
    ...
    console.info(
      `[setFile] File type is ${file.type}, falling back to jsmediatags`,
    );
  ...
  private async readAudioMetadata(file: File): Promise<JsMediaTagsTags> {
    return new Promise((res, rej) => {
      const reader = new JsMediaTagsReaderReal(file) as JsMediaTagsReader;
      reader.setTagsToRead(["title", "artist", "album"]).read({
        onSuccess: (data) => {
  ...
  setMp3MediaTags(tags?: MP3TagTags): void {
    if (typeof tags !== "object")
      throw new Error(
        `setMp3MediaTags: tags must be an object, ${typeof tags} given.`,
      );
  ...
  setAudioMediaTags(tags?: JsMediaTagsTags): void {
    if (typeof tags !== "object")
      throw new Error(
        `setAudioMediaTags: tags must be an object, ${typeof tags} given.`,
      );
  ...
  updateEntrySticks(): void {
    ...
    this.stateInd.setEntries(
      this.pluginScope.editor.getTimings(),
      this.audio.duration,
    );
  }

  setMetadataLength(): void {
    this.pluginScope.editor.setInitialMetadata({
      length: new Duration(this.audio.duration, true).toString(),
    });
  }
  ...
  recalcThumbPos(e: { clientX: number }): void {
    ...
    const relativeX = rangeLimit(
      e.clientX - pwOffset.left,
      false,
      0,
      pwWidth - 0.00001,
    );
