...
export class LRCParser {
  ...
  constructor(lrcFile: string) {
    ...
    if (file.length === 0)
      throw new Error(pageProps.jsLocales.dialog_parse_error_empty);
    ...
    lines.forEach((el) => {
      ...
      if (timestamps) {
        ...
        timestamps.forEach((ts) => {
          this.timings.push(
            new LRCString(text, ts.substring(1, ts.length - 1)),
          );
        });
      } else {
        ...
        if (metadata) {
          ...
          this.metadata[metadataName] =
            metadataName === "length" ? metadataValue.trim() : metadataValue;
        }
      }
    });
    if (this.timings.length === 0)
      throw new Error(pageProps.jsLocales.dialog_parse_error_no_timing);
