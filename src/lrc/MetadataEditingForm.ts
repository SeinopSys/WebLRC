...
export class MetadataEditingForm {
  ...
  constructor(compiledMetadata: LRCMetadata) {
    this.$form = $<HTMLFormElement>(document.createElement("form")).attr(
      "id",
      "metadata-editing-form",
    );
    const { metadata_field_placeholders } = pageProps.jsLocales;
    $.each(LRC_META_TAGS, (short, long) => {
      ...
      const longKey = (
        metadata_field_placeholders as unknown as Record<
          Required<LRCMetadata>[LRCMetadataKeys],
          string
        >
      )[long];
      this.$form.append(
        $(document.createElement("div"))
          ...
          .append(
            $(document.createElement("label"))
              .attr({ for: id })
              .append(longKey, ` <code>${short}</code>`),
      ...
      $(document.createElement("div"))
        ...
        .append(
          $(document.createElement("span")).text(
            pageProps.jsLocales.dialog_edit_meta_reset_info,
          ),
          ...
          $(document.createElement("button"))
            ...
            .append(
              ...
              $(document.createElement("span")).text(
                pageProps.jsLocales.dialog_edit_meta_reset_btn,
              ),
            ),
        ),
    );
