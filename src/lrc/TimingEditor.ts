...
import { binarySearch, clearFocus, LRC_META_TAGS, LRCMetadata } from "./common";
...
export class TimingEditor {
  ...
  constructor(private pluginScope: { player: AudioPlayer }) {
    ...
    this.$lrcpastebtn.on("click", (e) => {
      ...
      $rawLyricsForm.append(
        `<p class="text-info"><span class="fa fa-info-circle me-2"></span>${pageProps.jsLocales.dialog_pasteraw_info}</p>`,
      );
      Dialog.request({
        title: pageProps.jsLocales.dialog_pasteraw_title,
        content: $rawLyricsForm,
        confirmBtn: pageProps.jsLocales.dialog_pasteraw_action,
        callback: ($form) => {
          $form.on("submit", (se) => {
            se.preventDefault();
    ...
    this.$lrcimportaudiobtn.on("click", (e) => {
      ...
      if (!this.pluginScope.player.hasFile()) {
        Dialog.info(
          pageProps.jsLocales.dialog_import_audio_lyrics_title,
          pageProps.jsLocales.player_nofile,
        );
      }
    ...
    this.$lrcmetadatabtn.on("click", (e) => {
      ...
      Dialog.request({
        title: pageProps.jsLocales.dialog_edit_meta,
        content: $form,
        confirmBtn: pageProps.jsLocales.save,
        callback: () => {
          $form.on("submit", (se) => {
            se.preventDefault();
    ...
    this.$editor.on("click", ".addrow-up, .addrow-down", (e) => {
      ...
      const elClass = $(e.target).closest("button").attr("class") || "";
      const mathDirection = elClass.match(/addrow-(up|down)/);
      const insertWhere =
        mathDirection && mathDirection[1] === "up"
          ? "insertBefore"
          : "insertAfter";
      const $entry = this.makeEntryDiv(new LRCString())[insertWhere](
        $(e.target).closest(".time-entry"),
      );
    ...
    this.$editor.on("click", ".remrow", (e) => {
      ...
      if (
        isShiftKeyPressed(e) ||
        (entryText.length === 0 && timestampText.length === 0)
      ) {
      ...
      const timestampPrefix =
        timestampText.length > 0 ? `[${timestampText}] ` : "";
  ...
  private initBackup() {
    ...
    $(window).on("beforeunload", (e: BeforeUnloadEvent) => {
      if (!this.isEditorEmpty()) {
        ...
        e.preventDefault();
        e.returnValue = pageProps.jsLocales.confirm_navigation;
      }
    });
    ...
    this.$restoreBackupBtn.on("click", (e) => {
      ...
      $content
        .filter(".backup-data")
        .text(JSON.stringify(parsedBackupData, null, 2));
      ...
      Dialog.confirm({
        ...
        handlerFunc: (confirm) => {
          ...
          try {
            ...
            Dialog.fail(
              pageProps.jsLocales.backup_load_error,
  ...
  private createSpinnerTimeout(
    maxDelay: number,
    minDelay: number,
    stepsToReachMax: number,
    adjustFn: () => void,
  ) {
    const progressToMaxIterationsPercent =
      this.spinnerIterations / stepsToReachMax;
    const delayReduction =
      (maxDelay - minDelay) * progressToMaxIterationsPercent;
    this.spinnerTimeout = setTimeout(() => {
      adjustFn();
  ...
  changeMode(
    mode: "sync" | "edit" | null = null,
    preservePosition = false,
    forceChange = false,
  ): void {
    const localMode =
      mode == null ? (this.mode === "sync" ? "edit" : "sync") : mode;
    ...
    this.$editor.removeClass(`mode-${otherMode}`).addClass(`mode-${localMode}`);
    this.mode = localMode;
  ...
  setInitialMetadata(metadata: Partial<LRCMetadata>): void {
    ...
    if (metadata.lyrics) {
      const $audioLyricsForm = this.createLyricsImportForm(
        "audiolyrics",
        pageProps.jsLocales.dialog_import_audio_lyrics_info,
      );
      ...
      Dialog.request({
        title: pageProps.jsLocales.dialog_import_audio_lyrics_title,
        content: $audioLyricsForm,
        confirmBtn: pageProps.jsLocales.dialog_import_audio_lyrics_action,
        callback: ($form) => {
          $form.on("submit", (se) => {
            se.preventDefault();
  ...
  private createLyricsImportForm(id: string, pretext?: string) {
    const $audioLyricsForm = $<HTMLFormElement>(
  ...
  setMetadata(metadata: LRCMetadata): void {
    ...
    const metaCount = Object.keys(this.metadata).filter(
      (k) =>
        Boolean(this.metadata[k]) &&
        this.initialMetadata[k] !== this.metadata[k],
    ).length;
    this.$lrcmetadatabtn
      .find(".metadata-count")
      .text(metaCount > 0 ? metaCount : "");
  }
  ...
  storeTimings(backup = false): void {
    ...
    $children.each(function () {
      ...
      $.each(text, (_, el) => {
        timings.push(new LRCString(el, ts, $entry));
      });
    });
    this.timings = timings;
    if (!backup) {
  ...
  private adjustEntryTime(
    $entry: JQuery,
    direction: 1 | -1,
    secondsAmount: number = 0.1,
  ): void {
    ...
    const newDuration = new Duration(
      currentDuration.seconds + secondsAmount * direction,
    );
  ...
  updateEntryActionButtons(): void {
    ...
    setElDisabled(
      $toDisable.filter(function () {
        return Duration.isValid(
          $(this).parent().siblings().first().text().trim(),
        );
      }),
      false,
    );
  ...
  private updateTimestamp(
    $timestamp: JQuery,
    newValue: string | Duration | null,
  ): void {
    if (newValue === null) {
      $timestamp.empty();
    } else {
      $timestamp.text(
        newValue instanceof Duration ? newValue.toString() : newValue,
      );
    }
    ...
    return new Promise<boolean>((resolve) => {
      ...
      reader.onload = () => {
        try {
          ...
          Dialog.fail(
            pageProps.jsLocales.dialog_parse_error,
  ...
  hlEntry(position?: Duration): void {
    ...
    const i = binarySearch(
      usableTimings,
      (n) => n.ts.seconds - position.seconds,
    );
    if (i < 0) {
  ...
  private toggleMergeStrategy() {
    ...
    if (window.localStorage)
      localStorage.setItem(
        "lrc-merge",
        this.mergedOutputStrategy ? "true" : "false",
      );
    this.updateMergeStrategyButton();
  }
  ...
  private updateMergeStrategyButton() {
    ...
    $status.text(
      $status.attr(`data-${this.mergedOutputStrategy ? "true" : "false"}`) ||
        "",
    );
  }
  ...
  private exportEmbeddedAudioFile(lyrics: string) {
    ...
    if (!mp3Tag) {
      Dialog.fail(pageProps.jsLocales.timing_export_audio);
      return;
    }
  ...
  importFromText(input: string, forcePlaintext = true) {
    if (!forcePlaintext) {
      try {
        ...
        console.warn(e);
        console.warn(
          "Failed to parse input as LRC file, proceeding as plaintext",
        );
      }
    }
