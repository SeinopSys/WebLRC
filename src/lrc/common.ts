...
export const LRC_META_REGEX = new RegExp(
  `^\\[(${Object.keys(LRC_META_TAGS).join("|")}):([^\\]]+)]$`,
);
export type LRCMetadataKeys = keyof typeof LRC_META_TAGS;
export type LRCMetadata = Partial<Record<LRCMetadataKeys, string>> & {
  lyrics?: string;
} & { [k: string]: string };
