...
const dateFormatWithoutWeekday =
  `${isEnglishLocale ? "do MMMM yyyy" : "yyyy. MMMM do"}, ` +
    ...
    $("time[datetime]:not(.nodt)")
      ...
      .each(function () {
        ...
        if (typeof date !== "string")
          throw new TypeError(`Invalid date data type: "${typeof date}"`);
        ...
        const timeAgoStr = formatRelative(timestamp, now, {
          locale: dateFnsLocale,
        });
        const $elapsedHolder = $this.parent().children(".dynt-el");
        ...
        if ($elapsedHolder.length > 0 || $this.hasClass("no-dynt-el")) {
          $this.html(
            format(
              timestamp,
              showDayOfWeek ? dateformat.orderwd : dateformat.order,
              { locale: dateFnsLocale },
            ),
          );
          ...
          $this
            .attr(
              "title",
              format(timestamp, dateformat.order, { locale: dateFnsLocale }),
            )
            .html(timeAgoStr);
      });
  }
